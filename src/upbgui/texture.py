...
COLFMT_RGBA = 2
COLFMT_BGR = 3
def _getfilter4colfmt_bge(colfmt):
    return {
        COLFMT_GRAYSCALE: bge.texture.FilterGray,
        COLFMT_RGB: bge.texture.FilterRGB24,
        COLFMT_RGBA: bge.texture.FilterRGBA32,
        COLFMT_BGR: bge.texture.FilterBGR24
        }[colfmt]()
class Texture:
...
class ImageTexture(Texture):
    ...
    @classmethod
    def from_buffer(cls, arr, size, colorfmt, interp_mode=GL_LINEAR):
        self = cls(None, interp_mode, True)
        self.reload_buffer(arr, size, colorfmt)
        return self
    def reload_buffer(self, arr, size, colorfmt):
        width, height = tuple(size)
        if USING_BGE_TEXTURE:
            ibuf = bge.texture.ImageBuff()
            ibuf.filter = _getfilter4colfmt_bge(colorfmt)
            ibuf.flip = True
            ibuf.load(arr, width, height)
            data = ibuf.image
            if data is None:
