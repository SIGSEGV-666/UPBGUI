...
class Label(Widget):
    ...
    def __init__(self, parent, name=None, text="", font=None, pt_size=None, color=None,
                outline_color=None, outline_size=None, outline_smoothing=None, pos=[0, 0], clip_bounds=None, sub_theme='', options=BGUI_DEFAULT):
        """
    ...
    def _draw_text(self, x, y):
        for i, txt in enumerate(self._text.split("\n")):
    ...
    def _draw(self):
        ...
        if self.clip_bounds is not None:
            self.system.textlib.enable_clipping(self.fontid)
            self.system.textlib.clipping(self.fontid, *self.clip_bounds)
        self.system.textlib.size(self.fontid, self.pt_size, 72)
        ...
        if self.clip_bounds is not None:
            self.system.textlib.disable_clipping(self.fontid)
        Widget._draw(self)
